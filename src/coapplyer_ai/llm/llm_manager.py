...
class AzureOpenAIModel(AIModel):
    def __init__(self, api_key: str, azure_endpoint: str, azure_deployment: str, api_version: str):
        ...
        # Some Azure model deployments only accept default temperature=1.
        self.model = AzureChatOpenAI(**self._init_kwargs, temperature=1)

    def disable_temperature(self) -> None:
        """Reinitialize the model with temperature=1 for constrained Azure deployments."""
        from langchain_openai import AzureChatOpenAI
        self.model = AzureChatOpenAI(**self._init_kwargs, temperature=1)

    def invoke(self, prompt: str) -> BaseMessage:
        logger.debug("Invoking Azure OpenAI API")
        response = self.model.invoke(prompt)
        return response
...
class AIAdapter:
    ...
    def _create_model(self, config: dict, api_key: str) -> AIModel:
        ...
        elif llm_model_type == AZURE_OPENAI:
    ...
    def disable_temperature(self) -> bool:
        """Disable custom temperature for providers/models that require defaults."""
        if hasattr(self.model, "disable_temperature"):
            self.model.disable_temperature()
            return True

        target = self.model.model if hasattr(self.model, "model") else self.model
        if hasattr(target, "model_copy"):
            updated = target.model_copy(update={"temperature": None})
            if hasattr(self.model, "model"):
                self.model.model = updated
            else:
                self.model = updated
            return True
    ...
    def __call__(self, messages: List[Dict[str, str]]) -> str:
        ...
        while True:
            try:
                ...
                error_str = str(e)
                if 'temperature' in error_str.lower() and ('does not support' in error_str.lower() or 'unsupported_value' in error_str.lower()):
                    if not getattr(self, '_temperature_disabled', False):
                        logger.warning("Model does not support custom temperature, removing it and retrying.")
                        temperature_disabled = False
                        try:
                            if hasattr(self.llm, 'disable_temperature'):
                                temperature_disabled = bool(self.llm.disable_temperature())
                            else:
                                target = self.llm.model if hasattr(self.llm, 'model') else self.llm
                                if hasattr(target, 'model_copy'):
                                    updated = target.model_copy(update={"temperature": None})
                                    if hasattr(self.llm, 'model'):
                                        self.llm.model = updated
                                    else:
                                        self.llm = updated
                            ...
                            if temperature_disabled:
                                ...
                                logger.warning("Unable to disable temperature for this model wrapper.")
                                raise RuntimeError("Unable to disable temperature for this model wrapper.")
                        except Exception as patch_err:
                            logger.warning(f"Could not patch temperature: {patch_err}")
                            raise
                        continue  # retry immediately without sleeping
                    else:
                        logger.error("Temperature already removed but API still rejects request. Giving up on this call.")
                        raise
...
class GPTAnswerer:
    ...
    def set_job(self, job: Job):
        ...
        self.job = job
        if not (self.job.description or "").strip():
            logger.warning("Job description is empty; skipping summarization call")
